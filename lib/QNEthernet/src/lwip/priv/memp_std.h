...
#define LWIP_PBUF_MEMPOOL(name, num, payload, desc)        \
  LWIP_MEMPOOL(name, num,                                  \
               (LWIP_MEM_ALIGN_SIZE(sizeof(struct pbuf)) + \
                LWIP_MEM_ALIGN_SIZE(payload)),             \
               desc)
...
LWIP_MEMPOOL(ALTCP_PCB, MEMP_NUM_ALTCP_PCB, sizeof(struct altcp_pcb),
             "ALTCP_PCB")
...
LWIP_MEMPOOL(FRAG_PBUF, MEMP_NUM_FRAG_PBUF, sizeof(struct pbuf_custom_ref),
             "FRAG_PBUF")
#endif /* IP_FRAG && !LWIP_NETIF_TX_SINGLE_PBUF || (LWIP_IPV6 && \
          LWIP_IPV6_FRAG) */
...
#if LWIP_DNS
LWIP_MEMPOOL(DNS_API_MSG, MEMP_NUM_DNS_API_MSG, sizeof(struct dns_api_msg),
             "DNS_API_MSG")
#endif
...
LWIP_MEMPOOL(SELECT_CB, MEMP_NUM_SELECT_CB, sizeof(struct lwip_select_cb),
             "SELECT_CB")
...
LWIP_MEMPOOL(ND6_QUEUE, MEMP_NUM_ND6_QUEUE, sizeof(struct nd6_q_entry),
             "ND6_QUEUE")
...
LWIP_MEMPOOL(MLD6_GROUP, MEMP_NUM_MLD6_GROUP, sizeof(struct mld_group),
             "MLD6_GROUP")
 ...
 * pool_description) creates a pool name MEMP_pool_name. description is used in
 * stats.c This allocates enough space for the pbuf struct and a payload.
