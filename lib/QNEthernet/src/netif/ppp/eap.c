...
static void eap_send_failure(ppp_pcb *pcb) {
  ...
  p = pbuf_alloc(PBUF_RAW, (u16_t)(PPP_HDRLEN + EAP_HEADERLEN),
                 PPP_CTRL_PBUF_TYPE);
  if (NULL == p) return;
  if (p->tot_len != p->len) {
    pbuf_free(p);
...
static int eap_printpkt(const u_char *inp, int inlen,
  ...
  GETCHAR(id, inp);
  GETSHORT(len, inp);
  if (len < EAP_HEADERLEN || len > inlen) return (0);
        ...
        case EAPT_SRP:
          ...
          switch (vallen) {
            ...
            case EAPSRP_SVALIDATOR:
              ...
              if ((vallen = len) > SHA_DIGESTSIZE) vallen = SHA_DIGESTSIZE;
